...
func resourceUser() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			//TODO need revision
			"public_key": {
				Description: "Public key example for user. Need to be added better description.",
				Type:        schema.TypeString,
				Optional:    true,
			},
			//TODO need revision
			// "ssh_keys": {
			// 	Description: "SSH key array. Need to be added better description.",
...
func resourceUserUpdate(ctx context.Context, d *schema.ResourceData, meta interface{}) diag.Diagnostics {
	...
	payload := jcapiv1.Systemuserput{
		...
		Lastname:                    d.Get("lastname").(string),
		EnableUserPortalMultifactor: d.Get("enable_mfa").(bool),
		//SshKeys:                     d.Get("ssh_keys").([]jcapiv1.Sshkeypost),
		PublicKey: d.Get("public_key").(string),
	}
